...
**Last refreshed:** 2026-08-30T01:25:00Z

**Approve:** https://login.tailscale.com/a/1d0d805013867

Remint chain (do **not** use older links): `184ff33a` → `e064be30` → `6ad13a30` → **`1d0d8050`** (live). Forever `up --timeout=0s` armed. Tip through **#168** (CDN TIP_PIN hot-pick + standalone `hermes-ics-soft-hold.sh`; #167 TIP_PIN/CURRENT; #166 tip-stale soft-hold; #165/#164; #162 STALL/ONLY; #161 ENABLE; FALLBACK `b2b5fc4` tip159). **Ignore retired** predecessors + `80d5b860`, `7a69b1a0`, …

Prefer **Mac ONE-SHOT** tip #168 if Runtime Secrets stay unset:
...
Expect `zombie=1 zombie_passes=3` + tip through **#168**. Success = GitHub #1 machine `## Downstream DONE @ <ts>` + `host=` + **live** RAL-793 inventory (not SEED/`pending`/deferred).
