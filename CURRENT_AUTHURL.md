...
**Last refreshed:** 2026-08-30T04:50:46Z

**Approve:** https://login.tailscale.com/a/1d0d805013867

Remint chain (do **not** use older links): `184ff33a` → `e064be30` → `6ad13a30` → **`1d0d8050`** (live). Forever `up --timeout=0s` armed. Tip through **#170** (Dropbox public WAKE link + tip #169 soft-hold tick / Quo SMS). **Ignore retired** predecessors.
...
Expect `zombie=1 zombie_passes=3` + tip through **#170**. Success = GitHub #1 `## Downstream DONE @ <ts>` + `host=` + **live** RAL-793 inventory.
