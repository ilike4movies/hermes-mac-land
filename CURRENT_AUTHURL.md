...
Remint chain (do **not** use older links): `184ff33a` → `e064be30` → `6ad13a30` → **`1d0d8050`** (live). Forever `up --timeout=0s` armed. Tip through **#172** (NAG Path C reconnect ≤1/30m; #171 ONE-SHOT Path C tabs). **Ignore retired** predecessors.
...
Expect `zombie=1 zombie_passes=3` + tip through **#172** (ONE-SHOT + NAG open Path C reconnect). Success = GitHub #1 `## Downstream DONE @ <ts>` + `host=` + **live** RAL-793 inventory.

**Path C Zapier reconnects (Bad credentials / stale) — ONE-SHOT tip#171 + NAG tip#172 open these:**
