...
# Tip #147: prefers tip#142 once launcher, then tip/main dispatcher (no stale pin default).
...
echo "tip through #183 (ICS soft-hold auto-push tip#183; soft-hold dir tip#182; WAKE tip#180; Phase2 secrets tab tip#179; STALL nag tip#178; Path C tip#177; NAG Raw tip#176; FALLBACK b2b5fc4 tip159)"
...
_install_downstream_nag() {
  # Tip #178: STALL/ONLY skipped nag LaunchAgent install (ONE-SHOT step 0c had it).
  ...
  local NAG="/tmp/hermes-install-downstream-nag-stall-$$.command"
  local url
  rm -f "$NAG"
  for url in \
  ...
  do
    if curl -fsSL "$url" -o "$NAG" \
      ...
      echo "WARN nag installer exited non-zero — continuing STALL"
      rm -f "$NAG"
      return 0
    fi
    rm -f "$NAG"
  done
  echo "WARN could not fetch HERMES-INSTALL-DOWNSTREAM-NAG.command — continuing STALL"
  return 0
}
...
do
  ...
  if curl -fsSL "$url" -o "$ONCE" && _is_good_once "$ONCE"; then
    ...
    echo "OK tip#147 using once launcher: $url"
    set +e
    bash "$ONCE"
    rc=$?
    set -e
