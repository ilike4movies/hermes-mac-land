...
_post_github_status() {
  # Tip #156/#159: timeout gh; curl+token fallback; return non-zero on failure so
  # tip#159 callers can fail-closed when issue #1 Downstream DONE beacon is required.
  local body="$1"
  local posted=0
  ...
  # Tip #159: non-zero so callers can fail-closed when issue #1 beacon is required for obj5.
  echo "FAIL GitHub status post failed for ${GH_STATUS_REPO}#${GH_STATUS_ISSUE} (gh/token unavailable)" >&2
  return 1
}
