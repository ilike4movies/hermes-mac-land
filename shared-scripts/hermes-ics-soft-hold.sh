...
# Tip #174: durable soft-hold marker for agents + stamp CURRENT_AUTHURL.md ICS window.
SCRIPT_DIR="$SCRIPT_DIR" ICS_PATH="$ICS" TIP="$tip" URL="$url" NEED_TTL="$need_ttl" NEED_TIP="$need_tip" python3 - <<'PY' || true
import json, os, re
from datetime import datetime, timezone
base=os.environ.get("SCRIPT_DIR") or "."
ics=os.environ.get("ICS_PATH") or ""
tip=os.environ.get("TIP") or ""
url=os.environ.get("URL") or ""
need_ttl=os.environ.get("NEED_TTL") == "1"
need_tip=os.environ.get("NEED_TIP") == "1"
now=datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
dtend=""
summary=""
if ics and os.path.isfile(ics):
    text=open(ics, encoding="utf-8", errors="replace").read()
    m=re.search(r"^DTEND:([0-9]{8}T[0-9]{6}Z)$", text, re.M)
    sm=re.search(r"^SUMMARY:(.*)$", text, re.M)
    if m: dtend=m.group(1)
    if sm: summary=sm.group(1).strip()
kind="none"
if need_ttl: kind="ttl"
elif need_tip: kind="tip"
marker={
  "ts": now,
  "tip": tip,
  "url": url,
...
if os.path.isfile(cur) and dtend:
...
if kind != "none":
    print("ICS_SOFT_HOLD_REWRITE=1")
else:
    print("ICS_SOFT_HOLD_REWRITE=0")
PY
