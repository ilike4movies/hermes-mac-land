...
#
# Runs (in order):
#   1. RAL-793 contract install (--post-linear; does NOT dispatch)
...
bash "$_contract" --post-linear 2>&1 | tee -a "$LOG"
...
echo "  1. Confirm contract readback posted to RAL-793 (Step 1)" | tee -a "$LOG"
