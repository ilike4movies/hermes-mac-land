...
# Default direct .11 (jump grok-cos-1 is often down; Mac/cloud paths expect this).
...
if [[ "$RC" -ne 0 ]]; then
  if [[ -f "$BEACON" ]]; then
    bash "$BEACON" failed --source cloud-apply-install-via-ssh \
      --detail "land exited $RC prefer_direct=${HERMES_PREFER_DIRECT_HOST:-1} upload_tip=${UPLOAD_TIP_FROM_CALLER} jump=$JUMP_SSH host=$HOST_SSH" || true
  fi
