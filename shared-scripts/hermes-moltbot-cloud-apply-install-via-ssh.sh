...
echo "== cloud-apply-install-via-ssh jump=$JUMP_SSH dry_run=$DRY_RUN =="
...
ssh -o BatchMode=yes -o ConnectTimeout=20 "${SSH_IDENTITY_ARGS[@]}" "$JUMP_SSH" \
  env HERMES_MOLTBOT_REMOTE="$REMOTE_URL" bash -s <<'REMOTE'
set -euo pipefail
REMOTE_URL="${HERMES_MOLTBOT_REMOTE:-git@github.com:ilike4movies/moltbot.git}"
HTTPS_URL="https://github.com/ilike4movies/moltbot.git"
CLONE="/tmp/moltbot-main-tip-src"
rm -rf "$CLONE"
if ! git clone --depth 1 --branch main --single-branch "$REMOTE_URL" "$CLONE"; then
  echo "WARN: SSH clone failed; trying HTTPS"
  rm -rf "$CLONE"
  git clone --depth 1 --branch main --single-branch "$HTTPS_URL" "$CLONE"
fi
