...
if [[ "${HERMES_AUTO_SURGICAL_LAND:-1}" != "1" ]]; then
  ...
  # Tip #143: resolve via -f (+ CDN), not -x-only (0644 curl downloads).
  once=""
  for cand in \
    "$SCRIPT_DIR/hermes-cloud-run-downstream-once.sh" \
    "$SCRIPT_DIR/shared-scripts/hermes-cloud-run-downstream-once.sh"
  do
    if [[ -f "$cand" ]]; then
  ...
  if [[ -z "$once" ]]; then
    ...
    echo "tip#143 fetching hermes-cloud-run-downstream-once.sh"
  ...
  if [[ ! -f "$once" ]]; then
    echo "ERROR: missing hermes-cloud-run-downstream-once.sh (tip#143 local+CDN)" >&2
    exit 1
  fi
