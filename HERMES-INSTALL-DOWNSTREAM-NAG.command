...
# Does NOT auto-run ONE-SHOT (avoids unattended DISPATCH spam) — notification + opens issue #1.
...
osascript -e 'display notification "Still blocked — Terminal paste ONE-SHOT (see notification)" with title "Hermes Downstream" sound name "Basso"' 2>/dev/null || true
open "$ISSUE_URL" 2>/dev/null || true
# Also surface download page in browser so operator can save+Right-click Open
...
echo "  note: does NOT auto-run ONE-SHOT (notification + opens issue #1 / download URL)"
